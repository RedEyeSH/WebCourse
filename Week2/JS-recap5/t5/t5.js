...
function displayRestaurants(restaurants) {
  ...
  restaurants.forEach((restaurant) => {
    ...
    tr.addEventListener("click", async () => {
      ...
      const menuData = await fetchData(`${apiUrl}/restaurants/daily/${restaurant._id}/fi`);
...
function showModal(restaurant, menu, tr) {
  ...
  if (menu.length > 0) {
    menu.forEach((item) => {
      ...
      li.textContent = `${item.name} - ${item.price}€ (${item.diets.join(", ")})`;
      menuList.append(li);
    });
  } else {
  ...
  dialog.append(h3Name, pAddress, pCity, pPhoneNumber, menuTitle, menuList, closeButton);
  dialog.showModal();
}
